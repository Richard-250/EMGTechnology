...
import {Loader2, Truck, Store, Bike, MapPin, CalendarDays, Clock, Building2, Phone} from 'lucide-react';
...
export default function DeliveryStep({onComplete}: DeliveryStepProps) {
    ...
    // Map the 3 target delivery methods
    const deliveryOptions = useMemo<DisplayShippingOption[]>(() => {
        const findBackendMethod = (predicate: (m: {code?: string; name: string}) => boolean) => {
            return shippingMethods.find(predicate) || shippingMethods[0];
        };

        const motoMethod = findBackendMethod(m =>
            /moto|taxi/i.test(m.name) || /moto|taxi/i.test(m.code || '')
        );
        const pickupMethod = findBackendMethod(m =>
            /pickup|store|collect/i.test(m.name) || /pickup|store/i.test(m.code || '')
        );
        const expressMethod = findBackendMethod(m =>
            /express|2-hour|hour/i.test(m.name) || /express/i.test(m.code || '')
        );

        return [
            {
                key: 'moto',
                backendId: motoMethod?.id || 'moto',
            ...
            {
                key: 'pickup',
                backendId: pickupMethod?.id || 'pickup',
            ...
            {
                key: 'express',
                backendId: expressMethod?.id || 'express',
    ...
    const [selectedOptionKey, setSelectedOptionKey] = useState<'moto' | 'pickup' | 'express'>(() => {
    ...
    const isPickup = selectedOption.key === 'pickup';
    ...
    const handleContinue = async () => {
        setSubmitting(true);
        try {
            // Find best matching backend shipping method id
            const backendId = selectedOption.backendId;
            if (backendId && backendId !== 'moto' && backendId !== 'pickup' && backendId !== 'express') {
                await setShippingMethodAction(backendId);
            } else if (shippingMethods.length > 0) {
                // Fallback to first available backend shipping method if ids were synthetic
    ...
    return (
        <div className="space-y-6">
            {/* Delivery Zone Card (Nyereka Style) */}
            <div className="rounded-2xl border border-sky-200/90 dark:border-sky-800/50 bg-sky-50/70 dark:bg-sky-950/25 p-4 md:p-5 shadow-xs">
                <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3">
                        <span className="flex size-9 items-center justify-center rounded-full bg-sky-100 dark:bg-sky-900/60 text-sky-600 dark:text-sky-400 shrink-0 mt-0.5">
                    ...
                    <span className="text-xs font-semibold bg-sky-600 text-white px-3 py-1 rounded-full shrink-0 shadow-xs">
                        3 options
                    </span>
                </div>
            </div>

            {/* 3 Shipping Options (Radio Group) */}
            <RadioGroup
                value={selectedOptionKey}
                onValueChange={(val) => setSelectedOptionKey(val as 'moto' | 'pickup' | 'express')}
                className="space-y-3"
            >
                        ...
                        <Label
                            ...
                            className={cn(
                                ...
                                active
                                    ...
                                    : 'border-border/70 hover:border-sky-300 dark:hover:border-sky-700 bg-card hover:bg-muted/30',
                            )}
                        >
                            <RadioGroupItem value={option.key} id={`delivery-${option.key}`} className="shrink-0" />
                            <span
                                className={cn(
                            ...
                            <div className="font-bold shrink-0 text-sm md:text-base text-sky-600 dark:text-sky-400">
            ...
            {isPickup && (
                <div className="rounded-2xl border border-border/80 bg-muted/30 p-4 md:p-5 space-y-3">
                    ...
                    <div className="mt-3 rounded-xl bg-sky-100/60 dark:bg-sky-900/30 px-3 py-2 text-xs text-sky-800 dark:text-sky-300 font-medium">
                        ✓ No delivery fee. You will receive an SMS/Email notification when your package is ready.
                    </div>
                </div>
            )}

            {/* Delivery Date Section — shown only for Kigali Moto-taxi & Express delivery */}
            {!isPickup && (
                <div className="rounded-2xl border border-border/80 p-4 md:p-5 space-y-4 bg-card">
                        ...
                        <Label
                            className={cn(
                                ...
                                deliveryDateMode === 'ship-today'
                                    ...
                                    : 'border-border/70 hover:border-sky-300 dark:hover:border-sky-700 bg-card',
                            )}
                        >
                    ...
                    <div className="rounded-xl bg-muted/60 dark:bg-muted/30 px-4 py-3 text-sm space-y-2 border border-border/50">
                        ...
                        <div className="flex justify-between gap-2">
                            ...
                            <span className="font-bold text-xs md:text-sm text-sky-600 dark:text-sky-400">
                                {selectedOption.priceDisplayPrefix || ''}
            ...
            <Button
                ...
                className="w-full bg-sky-600 hover:bg-sky-700 text-white font-bold py-6 rounded-xl shadow-sm text-base transition-all"
            >
